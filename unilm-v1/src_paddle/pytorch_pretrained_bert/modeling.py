...
ACT2FN = {"gelu": gelu, "relu": paddle.nn.functional.relu, "swish": swish}
...
class BertLayerNorm(nn.Layer):
    ...
    def forward(self, x):
        ...
        x = (x - u) / paddle.sqrt(s + self.variance_epsilon)
...
class PositionalEmbedding(nn.Layer):
    ...
    def forward(self, pos_seq, bsz=None):
        sinusoid_inp = paddle.matmul(pos_seq, self.inv_freq,transpose_x = True)
...
class BertEmbeddings(nn.Layer):
    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None, task_idx=None):
        ...
        if self.num_pos_emb > 1:
            num_batch = position_embeddings.shape[0]
            num_pos = position_embeddings.shape[1]
            position_embeddings = position_embeddings.reshape([
                num_batch, num_pos, self.num_pos_emb, -1])[paddle.arange(0, num_batch).long(), :, task_idx, :]
...
class BertSelfAttention(nn.Layer):
    def __init__(self, config):
        ...
        self.query = nn.Linear(
            config.hidden_size, self.all_head_size*self.num_qkv, bias_attr=True)
        self.key = nn.Linear(config.hidden_size,
                             self.all_head_size*self.num_qkv, bias_attr=True)
        ...
        if self.uni_debug_flag:
            self.register_buffer('debug_attention_probs',
                                 paddle.zeros((512, 512)))
        if hasattr(config, 'seg_emb') and config.seg_emb:
            x = paddle.zeros(1, self.num_attention_heads, 1, self.attention_head_size)
            self.b_q_s = paddle.create_parameter(shape=x.shape,
                        dtype=str(x.numpy().dtype),
                        default_initializer=paddle.nn.initializer.Assign(x))
    ...
    def transpose_for_scores(self, x, mask_qkv=None):
        if self.num_qkv > 1:
            ...
            if mask_qkv is None:
                ...
                # mask_qkv: (batch, pos)
                if mask_qkv.shape[1] > sz[1]:
                    mask_qkv = mask_qkv[:, :sz[1]]
                # -> x: (batch, pos, head, head_hid)
                x = gather(x, 2, mask_qkv.reshape([sz[0], sz[1], 1, 1, 1]).expand([
                    sz[0], sz[1], 1, sz[3], sz[4]])).squeeze(2)
        else:
            sz = x.shape[:-1] + [self.num_attention_heads,
    ...
    def forward(self, hidden_states, attention_mask, history_states=None, mask_qkv=None, seg_ids=None):
        ...
        attention_scores = paddle.matmul(
            query_layer / math.sqrt(self.attention_head_size), key_layer.transpose([0,1,3,2]))

        if self.seg_emb is not None:
            seg_rep = self.seg_emb(seg_ids)
            # (batch, pos, head, head_hid)
            seg_rep = seg_rep.reshape([seg_rep.shape[0], seg_rep.shape[
                1], self.num_attention_heads, self.attention_head_size])
            qs = paddle.matmul((query_layer+self.b_q_s), seg_rep.transpose([0,2,3,1]))
        ...
        if self.uni_debug_flag:
            _pos = attention_probs.shape[-1]
            self.debug_attention_probs[:_pos, :_pos]= \
                attention_probs[0].mean(0).reshape([_pos, _pos]).clone()
...
class BertSelfOutput(nn.Layer):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.hidden_size, config.hidden_size, bias_attr=True)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-5)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertIntermediate(nn.Layer):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.hidden_size, config.intermediate_size, bias_attr=True)
...
class BertOutput(nn.Layer):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.intermediate_size, config.hidden_size, bias_attr=True)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-5)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class TransformerFFN(nn.Layer):
    def __init__(self, config):
        ...
        if self.ffn_type in (1, 2):
            self.wx0 = nn.Linear(config.hidden_size, config.hidden_size, bias_attr=True)
        if self.ffn_type in (2,):
            self.wx1 = nn.Linear(config.hidden_size, config.hidden_size, bias_attr=True)
        if self.ffn_type in (1, 2):
            self.output = nn.Linear(config.hidden_size, config.hidden_size, bias_attr=True)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-5)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertEncoder(nn.Layer):
    def __init__(self, config):
        ...
        self.layer = nn.LayerList([copy.deepcopy(layer)
...
class BertLMPredictionHead(nn.Layer):
    def __init__(self, config, bert_model_embedding_weights, **kwargs):
        super(BertLMPredictionHead, self).__init__(**kwargs)
        self.transform = BertPredictionHeadTransform(config)

        # The output weights are the same as the input embeddings, but there is
        # an output-only bias for each token.
        self.decoder = nn.Linear(bert_model_embedding_weights.shape[0],
...
class BertOnlyNSPHead(nn.Layer):
    def __init__(self, config):
        ...
        self.seq_relationship = nn.Linear(config.hidden_size, 2, bias_attr=True)
...
class BertPreTrainingHeads(nn.Layer):
    def __init__(self, config, bert_model_embedding_weights, num_labels=2):
        ...
        self.predictions = BertLMPredictionHead(
            config, bert_model_embedding_weights)
        self.seq_relationship = nn.Linear(config.hidden_size, num_labels, bias_attr=True)
...
class PreTrainedBertModel(nn.Layer):
    ...
    def from_pretrained(cls, pretrained_model_name, state_dict=None, cache_dir=None, *inputs, **kwargs):
        ...
        if (_k in state_dict) and (config.type_vocab_size != state_dict[_k].shape[0]):
            ...
            if config.type_vocab_size > state_dict[_k].shape[0]:
                ...
                # print(state_dict[_k].shape)
                # print((config.type_vocab_size,state_dict[_k].shape[1]))
                state_cache = state_dict[_k].astype('float32')
                print("!!!:", state_cache.dtype)
                state_dict[_k] = paddle.zeros(shape=[config.type_vocab_size, state_dict[_k].shape[1]],dtype=state_cache.dtype)
                state_dict[_k][0:state_cache.shape[0], 0:state_cache.shape[1]]=state_cache
                # L2R
                if config.type_vocab_size >= 3:
                    # state_dict[_k].data[2, :].copy_(state_dict[_k].data[0, :])
                    paddle.assign(state_dict[_k][0, :], state_dict[_k][2, :])
                # R2L
                if config.type_vocab_size >= 4:
                    # state_dict[_k].data[3, :].copy_(state_dict[_k].data[0, :])
                    paddle.assign(state_dict[_k][0, :], state_dict[_k][3, :])
                # S2S
                if config.type_vocab_size >= 6:
                    ...
                    paddle.assign(state_dict[_k][0, :], state_dict[_k][4, :])
                    paddle.assign(state_dict[_k][1, :], state_dict[_k][5, :])
                if config.type_vocab_size >= 7:
                    # state_dict[_k].data[6, :].copy_(state_dict[_k].data[1, :])
                    paddle.assign(state_dict[_k][1, :], state_dict[_k][6, :])
            elif config.type_vocab_size < state_dict[_k].shape[0]:
                state_dict[_k] = state_dict[_k][:config.type_vocab_size, :]
        ...
        if (_k in state_dict) and (n_config_pos_emb*config.hidden_size != state_dict[_k].shape[1]):
            ...
            if n_state_pos_emb == 1:
                state_dict[_k] = paddle.fluid.layers.expand(state_dict[_k].unsqueeze(1),[1, n_config_pos_emb, 1]
                            ).reshape([(config.max_position_embeddings, n_config_pos_emb*config.hidden_size)])
                ...
                if hasattr(config, 'task_idx') and (config.task_idx is not None) and (0 <= config.task_idx <= 3):
                    ...
                    _task_idx = 0
                state_dict[_k] = state_dict[_k].reshape([
        ...
        if _k in state_dict and config.max_position_embeddings != state_dict[_k].shape[0]:
            ...
            if config.max_position_embeddings > state_dict[_k].shape[0]:
                ...
                state_cache = state_dict[_k].astype('float32')
                state_dict[_k] = paddle.zeros(shape=[config.max_position_embeddings, state_dict[_k].shape[1]],dtype=state_cache.dtype)
                state_dict[_k][0:state_cache.shape[0], 0:state_cache.shape[1]] = state_cache
                start = old_size
                while start < config.max_position_embeddings:
                    ...
                    paddle.assign(state_dict[_k][:chunk_size, :], state_dict[_k][start:start+chunk_size, :])
                    start += chunk_size
        ...
        if (_k in state_dict) and (n_config_relax*config.hidden_size != state_dict[_k].shape[0]):
            ...
            if n_state_relax == 1:
                _k = 'cls.predictions.transform.dense.weight'
                state_dict[_k] = paddle.fluid.layers.expand(state_dict[_k].unsqueeze(0), [n_config_relax, 1, 1]
                        ).reshape((n_config_relax*config.hidden_size, config.hidden_size))
                for _k in ('cls.predictions.transform.dense.bias', 'cls.predictions.transform.LayerNorm.weight', 'cls.predictions.transform.LayerNorm.bias'):
                    state_dict[_k] = paddle.fluid.layers.expand(state_dict[_k].unsqueeze(
                        0),[n_config_relax, 1]).reshape([-1])
            ...
            if (_k in state_dict) and (n_config_num_qkv*_all_head_size != state_dict[_k].shape[0]):
                ...
                for layer_idx in range(config.num_hidden_layers):
                    ...
                    if n_state_qkv == 1:
                        _k = 'bert.encoder.layer.{0}.attention.self.{1}.weight'.format(
                            layer_idx, qkv_name)
                        state_dict[_k] = paddle.fluid.layers.expand(state_dict[_k].unsqueeze(0),[n_config_num_qkv, 1, 1]
                                        ).reshape((n_config_num_qkv*_all_head_size, _all_head_size))
                        _k = 'bert.encoder.layer.{0}.attention.self.{1}.bias'.format(
                            layer_idx, qkv_name)
                        state_dict[_k] = paddle.fluid.layers.expand(state_dict[_k].unsqueeze(
                            0), [n_config_num_qkv, 1]).reshape([-1])
        ...
        if _word_emb_map:
            ...
            for _tgt, _src in _word_emb_map:
                ...
                paddle.assign(state_dict[_k][_src, :], state_dict[_k][_tgt, :])
        missing_keys = []
        unexpected_keys = []
        error_msgs = []
...
class BertModel(PreTrainedBertModel):
    ...
    def rescale_some_parameters(self):
        for layer_id, layer in enumerate(self.encoder.layer):
            layer.attention.output.dense.weight.divide(
                paddle.to_tensor(math.sqrt(2.0*(layer_id + 1))))
            layer.output.dense.weight.divide(paddle.to_tensor(math.sqrt(2.0*(layer_id + 1))))
    ...
    def get_extended_attention_mask(self, input_ids, token_type_ids, attention_mask):
        ...
        extended_attention_mask = extended_attention_mask.astype(
            dtype='float32')  # fp16 compatibility  original:next(self.parameters()).dtype
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, next_sentence_label=None, mask_qkv=None, task_idx=None):
        ...
        if masked_lm_labels is not None and next_sentence_label is not None:
            ...
            masked_lm_loss = loss_fct(
                prediction_scores.reshape([-1, self.config.vocab_size]), masked_lm_labels.reshape(-1))
...
class BertPreTrainingPairTransform(nn.Layer):
    def __init__(self, config):
        super(BertPreTrainingPairTransform, self).__init__()
        self.dense = nn.Linear(config.hidden_size*2, config.hidden_size, bias_attr=True)
        self.transform_act_fn = ACT2FN[config.hidden_act] \
            if isinstance(config.hidden_act, str) else config.hidden_act
        # self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-5)

    def forward(self, pair_x, pair_y):
        hidden_states = paddle.concat([pair_x, pair_y], axis=-1)
        hidden_states = self.dense(hidden_states)
        hidden_states = self.transform_act_fn(hidden_states)
        # hidden_states = self.LayerNorm(hidden_states)
        return hidden_states
...
class BertPreTrainingPairRel(nn.Layer):
    ...
    def forward(self, pair_x, pair_y, pair_r, pair_pos_neg_mask):
        ...
        _batch, _num_pair, _hidden = xy.shape
...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2, num_rel=0, num_sentlvl_labels=0, no_nsp=False):
        ...
        self.crit_mask_lm = nn.CrossEntropyLoss(reduction='none', axis = 1)
        if no_nsp:
        ...
        if hasattr(config, 'label_smoothing') and config.label_smoothing:
            self.crit_mask_lm_smoothed = LabelSmoothingLoss(
                config.label_smoothing, config.vocab_size, ignore_index=0, size_average=None, reduction='none')
        else:
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        if token_type_ids is None and attention_mask is None:
            ...
            index_matrix = paddle.arange(sequence_length).reshape([
                1, sequence_length]).cuda(input_ids.device)

            num_tokens = num_tokens_a + num_tokens_b

            base_mask = (index_matrix < num_tokens.reshape([-1, 1])
                        ).cast(input_ids)
            segment_a_mask = (
                index_matrix < num_tokens_a.reshape([-1, 1])).cast(input_ids)

            token_type_ids = (
                task_idx + 1 + task_3.cast(task_idx)).reshape([-1, 1]) * base_mask
            token_type_ids = token_type_ids - segment_a_mask * \
                (task_0 | task_3).cast(segment_a_mask).reshape([-1, 1])

            index_matrix = index_matrix.reshape([1, 1, sequence_length])
            index_matrix_t = index_matrix.reshape([1, sequence_length, 1])

            tril = index_matrix <= index_matrix_t

            attention_mask_task_0 = (
                index_matrix < num_tokens.reshape([-1, 1, 1])) & (index_matrix_t < num_tokens.reshape([-1, 1, 1]))
            attention_mask_task_1 = tril & attention_mask_task_0
            attention_mask_task_2 = paddle.transpose(
                tril, dim0=-2, dim1=-1) & attention_mask_task_0
            attention_mask_task_3 = (
                (index_matrix < num_tokens_a.reshape([-1, 1, 1])) | tril) & attention_mask_task_0

            attention_mask = (attention_mask_task_0 & task_0.reshape([-1, 1, 1])) | \
                             (attention_mask_task_1 & task_1.reshape([-1, 1, 1])) | \
        ...
        def gather_seq_out_by_pos_average(seq, pos, mask):
            ...
            batch_size, max_token_num = pos.shape[0], pos.shape[-1]
            # (batch, num_pair, max_token_num, seq.size(-1))
            pos_vec = gather(seq, 1, pos.reshape([batch_size, -1]).unsqueeze(
                2).expand([-1, -1, seq.shape[-1]])).reshape([batch_size, -1, max_token_num, seq.shape[-1]])
            # (batch, num_pair, seq.size(-1))
            mask = mask.cast(pos_vec)
            pos_vec_masked_sum = (
        ...
        def loss_mask_and_normalize(loss, mask):
            mask = mask.cast(loss.dtype)
            loss = loss * mask
            denominator = paddle.sum(mask) + 1e-5
        ...
        if self.crit_next_sent is None or next_sentence_label is None:
            ...
            next_sentence_loss = self.crit_next_sent(
                seq_relationship_score.reshape([-1, self.num_labels]).astype('float'), next_sentence_label.reshape([-1]))
        ...
        if self.cls2 is not None and masked_pos_2 is not None:
            ...
            masked_lm_loss_2 = self.crit_mask_lm(
                prediction_scores_masked_2.transpose(1, 2).astype('float'), masked_labels_2)
            masked_lm_loss_2 = loss_mask_and_normalize(
                masked_lm_loss_2.astype('float'), masked_weights_2)
        ...
        pair_loss = loss_mask_and_normalize(
            pair_loss.astype('float'), pair_loss_mask)
...
class BertForExtractiveSummarization(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_pos_2=None, masked_weights_2=None, task_idx=None, mask_qkv=None):
        ...
        predicted_probs = paddle.nn.functional.softmax(
            prediction_scores_masked_2, axis=-1)
...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None):
        ...
        output_shape = list(token_type_ids.shape)
        ...
        curr_ids = input_ids
        mask_ids = paddle.full((batch_size, 1), self.mask_word_id)
        next_pos = input_length
        if self.pos_shift:
            sos_ids = paddle.full((batch_size, 1), self.sos_id)

        while next_pos < output_length:
            curr_length = list(curr_ids.shape)[1]

            if self.pos_shift:
                if next_pos == input_length:
                    x_input_ids = paddle.concat((curr_ids, sos_ids), axis=1)
                    start_pos = 0
                else:
                ...
                x_input_ids = paddle.concat((curr_ids, mask_ids), axis=1)
            ...
            if self.not_predict_set:
                for token_id in self.not_predict_set:
                    prediction_scores[:, :, token_id] = paddle.full_like(prediction_scores[:, :, token_id], -10000.0)
            _, max_ids = paddle.max(prediction_scores, axis=-1)
            ...
            if self.pos_shift:
                if prev_embedding is None:
                    ...
                    prev_embedding = paddle.concat(
                        (prev_embedding, new_embedding), axis=1)
                if prev_encoded_layers is None:
                    prev_encoded_layers = [x for x in new_encoded_layers]
                else:
                    prev_encoded_layers = [paddle.concat((x[0], x[1]), axis=1) for x in zip(
                ...
                if prev_embedding is None:
                    ...
                    prev_embedding = paddle.concat(
                        (prev_embedding, new_embedding[:, :-1, :]), axis=1)
                if prev_encoded_layers is None:
                    prev_encoded_layers = [x[:, :-1, :]
                                           for x in new_encoded_layers]
                else:
                    prev_encoded_layers = [paddle.concat((x[0], x[1][:, :-1, :]), axis=1)
                                           for x in zip(prev_encoded_layers, new_encoded_layers)]
            curr_ids = max_ids
            next_pos += 1

        return paddle.concat(output_ids, axis=1)
    ...
    def beam_search(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None):
        ...
        while next_pos < output_length:
            ...
            prediction_scores, _ = self.cls(
                last_hidden, None, task_idx=task_idx)
            log_scores = paddle.nn.functional.log_softmax(
                prediction_scores, dim=-1)
            if forbid_word_mask is not None:
                log_scores += (forbid_word_mask * -10000.0)
            if self.min_len and (next_pos-input_length+1 <= self.min_len):
                log_scores[:, :, self.eos_id]=paddle.full_like(log_scores[:, :, self.eos_id], -10000.0)
            if self.not_predict_set:
                for token_id in self.not_predict_set:
                    log_scores[:, :, token_id]=paddle.full_like(log_scores[:, :, token_id], -10000.0)
            kk_scores, kk_ids = paddle.topk(log_scores, k=K)
            if len(total_scores) == 0:
                k_ids = paddle.reshape(kk_ids, [batch_size, K])
                back_ptrs = paddle.zeros(batch_size, K, dtype=paddle.long)
                k_scores = paddle.reshape(kk_scores, [batch_size, K])
            else:
                last_eos = paddle.reshape(
                    beam_masks[-1], [batch_size * K, 1, 1])
                last_seq_scores = paddle.reshape(
                    total_scores[-1], [batch_size * K, 1, 1])
                kk_scores += last_eos * (-10000.0) + last_seq_scores
                kk_scores = paddle.reshape(kk_scores, [batch_size, K * K])
                k_scores, k_ids = paddle.topk(kk_scores, k=K)
                back_ptrs = paddle.divide(k_ids, K)
                kk_ids = paddle.reshape(kk_ids, [batch_size, K * K])
                k_ids = gather(kk_ids, 1, k_ids)
            step_back_ptrs.append(back_ptrs)
            step_ids.append(k_ids)
            beam_masks.append(paddle.equal(k_ids, self.eos_id).float())
            total_scores.append(k_scores)

            def first_expand(x):
                input_shape = list(x.shape)
                expanded_shape = input_shape[:1] + [1] + input_shape[1:]
                x = paddle.reshape(x, expanded_shape)
                repeat_count = [1, K] + [1] * (len(input_shape) - 1)
                x = paddle.tile(x, repeat_count)
                x = paddle.reshape(x, [input_shape[0] * K] + input_shape[1:])
                return x
            ...
            def select_beam_items(x, ids):
                ...
                x = paddle.reshape(x, [batch_size, K] + x_shape[1:])
            ...
            curr_ids = paddle.reshape(k_ids, [batch_size * K, 1])
            ...
            if self.forbid_duplicate_ngrams:
                wids = paddle.tolist(step_ids[-1])
                ptrs = paddle.tolist(step_back_ptrs[-1])
                if is_first:
                    partial_seqs = []
                ...
                if len(partial_seqs[0]) >= self.ngram_size:
                    ...
                    if max(len(x) for x in dup_cands) > 0:
                        if buf_matrix is None:
                            vocab_size = list(log_scores.shape)[-1]
                            buf_matrix = np.zeros(
                        ...
                        forbid_word_mask = paddle.to_tensor(
                            buf_matrix, dtype=log_scores.dtype)
                        forbid_word_mask = paddle.reshape(
        ...
        def _pad_sequence(sequences, max_len, padding_value=0):
            ...
            out_tensor = sequences[0].data.full(out_dims, padding_value)
        ...
        for k in ('pred_seq', 'scores', 'wids', 'ptrs'):
            ts_list = traces[k]
            if not isinstance(ts_list[0], paddle.Tensor):
                dt = 'float32' if k == 'scores' else 'int64'
                ts_list = [paddle.to_tensor(it, dtype=dt) for it in ts_list]
            traces[k] = _pad_sequence(
                ts_list, output_length, padding_value=0).cuda(input_ids.device)
...
class BertForSequenceClassification(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, mask_qkv=None, task_idx=None):
        ...
        if labels is not None:
            ...
            elif labels.dtype == 'float16' or labels.dtype == 'float32':
                loss_fct = MSELoss()
...
class BertForMultipleChoice(PreTrainedBertModel):
    ...
    def __init__(self, config, num_choices=2):
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, 1, bias_attr=True)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, mask_qkv=None, task_idx=None):
        flat_input_ids = input_ids.reshape([-1, input_ids.shape[-1]])
        flat_token_type_ids = token_type_ids.reshape([-1, token_type_ids.shape[-1]])
...
class BertForQuestionAnswering(PreTrainedBertModel):
    ...
    def __init__(self, config):
        ...
        self.bert = BertModel(config)
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.qa_outputs = nn.Linear(config.hidden_size, 2, bias_attr=True)
        self.apply(self.init_bert_weights)
