...
from src_paddle.biunilm.loader_utils import get_random_word, batch_list_to_batch_tensors, Pipeline
...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __init__(self, max_pred, mask_prob, vocab_words, indexer, max_len=512, skipgram_prb=0, skipgram_size=0, block_mask=False, mask_whole_word=False, new_segment_ids=False, truncate_config={}, mask_source_words=False, mode="s2s", has_oracle=False, num_qkv=0, s2s_special_token=False, s2s_add_segment=False, s2s_share_segment=False, pos_shift=False):
        ...
        self.max_len = max_len
        self._tril_matrix = paddle.tril(paddle.ones(
            (max_len, max_len), dtype='int64'))
    ...
    def __call__(self, instance):
        ...
        input_mask = paddle.zeros([self.max_len, self.max_len], dtype='int64')
        if self.mode == "s2s":
            input_mask[:, :len(tokens_a)+2] = paddle.full_like(input_mask[:, :len(tokens_a)+2], 1)
            second_st, second_end = len(
                tokens_a)+2, len(tokens_a)+len(tokens_b)+3
            # input_mask[second_st:second_end, second_st:second_end].copy_(
            #     self._tril_matrix[:second_end-second_st, :second_end-second_st])
            paddle.assign(self._tril_matrix[:second_end-second_st, :second_end-second_st], 
                        input_mask[second_st:second_end, second_st:second_end])
        else:
            st, end = 0, len(tokens_a) + len(tokens_b) + 3
            input_mask[st:end, st:end] = paddle.assign(self._tril_matrix[:end, :end])
...
class Preprocess4Seq2seqDecoder(Pipeline):
    ...
    def __call__(self, instance):
        ...
        # Zero Padding
        input_mask = paddle.zeros(
            [max_len_in_batch, max_len_in_batch], dtype='int64')
        if self.mode == "s2s":
            input_mask[:, :len(tokens_a)+2] = paddle.full_like(input_mask[:, :len(tokens_a)+2], 1)
        else:
            ...
            input_mask[end:, :len(tokens_a)+2] = paddle.full_like(input_mask[end:, :len(tokens_a)+2], 1)
        second_st, second_end = len(padded_tokens_a), max_len_in_batch

        # input_mask[second_st:second_end, second_st:second_end].copy_(
        #     self._tril_matrix[:second_end-second_st, :second_end-second_st])
        paddle.assign(self._tril_matrix[:second_end-second_st, :second_end-second_st], input_mask[second_st:second_end, second_st:second_end])
