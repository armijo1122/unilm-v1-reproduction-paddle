...
WEIGHTS_NAME = 'pytorch_model32.bin'
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, history_states=None, mask_qkv=None, seg_ids=None):
        if history_states is None:
            log_torch.add("BertSelfAttention_hidden_states",hidden_states.cpu().detach().numpy())
            mixed_query_layer = self.query(hidden_states)
            mixed_key_layer = self.key(hidden_states)
            mixed_value_layer = self.value(hidden_states)
            log_torch.add("BertSelfAttention_query",mixed_query_layer.cpu().detach().numpy())
            log_torch.add("BertSelfAttention_key",mixed_key_layer.cpu().detach().numpy())
            log_torch.add("BertSelfAttention_value", mixed_value_layer.cpu().detach().numpy())
        else:
            log_torch.add("BertSelfAttention_hidden_states",hidden_states.cpu().detach().numpy())
            x_states = torch.cat((history_states, hidden_states), dim=1)
            log_torch.add("BertSelfAttention_x_states",x_states.cpu().detach().numpy())
            mixed_query_layer = self.query(hidden_states)
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, history_states=None, mask_qkv=None, seg_ids=None):
        ...
        log_torch.add("BertAttention_self_output", self_output.cpu().detach().numpy())
        attention_output = self.output(self_output, input_tensor)
        log_torch.add("BertAttention_attention_output", attention_output.cpu().detach().numpy())
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, history_states=None, mask_qkv=None, seg_ids=None):
        ...
        log_torch.add("BertLayer_attention_output", attention_output.cpu().detach().numpy())
        if self.ffn_type:
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, prev_embedding=None, prev_encoded_layers=None, mask_qkv=None, seg_ids=None):
        ...
        log_torch.add("BertEncoder_hidden_states", hidden_states.cpu().detach().numpy())
        all_encoder_layers = []
...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        log_torch.add("Bert_Pooler_first_token_tensor", first_token_tensor.cpu().detach().numpy())
        pooled_output = self.dense(first_token_tensor)
        log_torch.add("Bert_Pooler_pooled_output", pooled_output.cpu().detach().numpy())
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, mask_qkv=None, task_idx=None):
        ...
        log_torch.add("Bert_Model_extended_attenrion_mask",extended_attention_mask.cpu().detach().numpy())
        embedding_output = self.embeddings(
            input_ids, token_type_ids, task_idx=task_idx)
        log_torch.add("Bert_Model_embedding_output", embedding_output.cpu().detach().numpy())
        ...
        log_torch.add("Bert_Model_sequence_output", sequence_output.cpu().detach().numpy())
        # print("!!!tui:", sequence_output)
        pooled_output = self.pooler(sequence_output)
        log_torch.add("Bert_Model_pooled_output", pooled_output.cpu().detach().numpy())
        if not output_all_encoded_layers:
...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        log_torch.add("input_ids", input_ids.cpu().detach().numpy())
        log_torch.add("token_type_ids", token_type_ids.cpu().detach().numpy())
        log_torch.add("attention_mask", attention_mask.cpu().detach().numpy())
        # log_torch.add("mask_qkv", mask_qkv.cpu().detach().numpy())
        # log_torch.add("task_idx", task_idx.cpu().detach().numpy())
        sequence_output, pooled_output = self.bert(
            input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, mask_qkv=mask_qkv, task_idx=task_idx)
        log_torch.add("sequence_output", sequence_output.cpu().detach().numpy())
        log_torch.add("pooled_output", pooled_output.cpu().detach().numpy())
        ...
        if masked_lm_labels is None:
            ...
            log_torch.add("prediction_scores", prediction_scores.cpu().detach().numpy())
            log_torch.add("seq_relationship_score", seq_relationship_score.cpu().detach().numpy())
            log_torch.save("/lustre/S/fuqiang/unilm/unilm/unilm-v1/bert-cased-pretrained-cache/torch_forward.npy")
            return prediction_scores, seq_relationship_score
