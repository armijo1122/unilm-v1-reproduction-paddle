...
random_input = torch.tensor([[5, 6, 1], [3, 5, 0]], dtype = torch.long).cuda()
token_type_ids = torch.tensor([[0, 0, 1], [0, 1, 0]], dtype = torch.long).cuda()
input_mask = torch.tensor([[1, 1, 1], [1, 1, 0]], dtype = torch.long).cuda()
...
# loss
lm_label_ids = torch.tensor([[0, 0, 1], [0, 1, 0]], dtype = torch.long).cuda()
masked_pos = torch.tensor([[0, 0, 1], [1, 1, 0]], dtype = torch.long).cuda()
masked_weights = torch.tensor([[0.1,0.24,0.6], [0.3, 0.51, 0.8]], dtype = torch.float, requires_grad = True).cuda()
