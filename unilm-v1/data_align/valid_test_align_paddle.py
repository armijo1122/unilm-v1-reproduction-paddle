...
import paddle
from paddle.io import SequenceSampler, BatchSampler

from src_paddle.pytorch_pretrained_bert.tokenization import BertTokenizer as BertTokenizerp
from src_paddle.pytorch_pretrained_bert.tokenization import WhitespaceTokenizer as WhitespaceTokenizerp
...
import src_paddle.biunilm.seq2seq_loader as seq2seq_loaderp
...
def build_paddle_data_pipeline():
     ...
     tokenizer = BertTokenizerp.from_pretrained(
          args.bert_model, do_lower_case=args.do_lower_case)

     if args.max_position_embeddings:
          tokenizer.max_len = args.max_position_embeddings
     data_tokenizer = WhitespaceTokenizerp() if args.tokenized_input else tokenizer

     print("Loading Train Dataset", args.data_dir)
     bi_uni_pipeline = [seq2seq_loaderp.Preprocess4Seq2seq(args.max_pred, args.mask_prob, list(tokenizer.vocab.keys(
     ...
     file_oracle = None
     test_dataset = seq2seq_loaderp.Seq2SeqDataset(
     ...
     test_sampler = paddle.io.SequenceSampler(test_dataset)
     _batch_size = args.train_batch_size
     test_batch_sampler = paddle.io.BatchSampler(sampler = test_sampler, batch_size = _batch_size)
     test_dataloader = paddle.io.DataLoader(test_dataset, batch_sampler = test_batch_sampler,
                                             num_workers=args.num_workers, collate_fn=seq2seq_loaderp.batch_list_to_batch_tensors, use_shared_memory=True)
     return test_dataset, test_dataloader

if __name__ =="__main__":
    paddle_dataset, paddle_dataloader= build_paddle_data_pipeline()
    log_paddle.add("length", np.array(len(paddle_dataset)))

    for idx in range(5):
        log_paddle.add(f"dataset_{idx}",
                         np.array(paddle_dataset[idx][0]))
    
    for idx, paddle_batch in enumerate(paddle_dataloader):
        if idx >= 5:
            break
        log_paddle.add(f"dataloader_{idx}", paddle_batch[0].numpy())

    log_paddle.save("paddle_data.npy")